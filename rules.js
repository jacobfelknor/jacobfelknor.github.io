...
function begin_play(){
	if(!game_started()){
		var player_1_field = document.getElementById("player1_id");
		var player_2_field = document.getElementById("player2_id");
		console.log("Player 1 name: " + player_1_field.value);
		console.log("Player 2 name: " + player_2_field.value);
		if(isEmpty(player_1_field.value) || isEmpty(player_2_field.value)){
			alert('Player\'s fields cannot be empty!');
		}else{
			player_1_field.value += " (X)";
			player_1_field.disabled = true;
			player_2_field.value += " (O)";
			player_2_field.disabled = true;
...
function reset_play(){
	...
	player_1_field.disabled = false;
	player_2_field.value = "";
	player_2_field.disabled = false;
	var turn_info = document.getElementById("turn_info");
	turn_info.innerHTML = "Game has not begin.";
}

/* 
	Helper function used in play. Determines if there is a winner
*/
function check_for_winner(){
	var winning_combos = [[0,1,2], [0,4,8], [0,3,6], [1,4,7], [2,5,8], [2,4,6], [3,4,5], [6,7,8]];
...
function play() {
	if(game_started()){
		...
		if(!table_ids.includes(move.value)){
			alert("Invalid move! Enter a location on the grid");
		}else{
			...
			if (whose_move()){
				...
				if(board_state[index] == -1){
					board_state[index] = 1;
					console.log("player 1 chose: " + move.value);
					table_item = document.getElementById(move.value);
					table_item.innerHTML = "X";
				...
				if(board_state[index] == -1){
					board_state[index] = 0;
					console.log("player 2 chose: " + move.value);
					table_item = document.getElementById(move.value);
					table_item.innerHTML = "O";
